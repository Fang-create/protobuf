...
void start(void)
{
	...
	while(1)
	{
		...
		if(login_msg++ % 2 == 0) 
		{
		    ...
		    cPdu.SetServiceId(IM::BaseDefine::SID_LOGIN);   //往buf + 8
		    cPdu.SetCommandId(IM::BaseDefine::CID_LOGIN_REQ_USERLOGIN);   //往buf + 10
			
		    uint32_t nSeqNo = s_seq_no++;
		    cPdu.SetSeqNum(nSeqNo);    // CByteStream::WriteUint16(buf + 12, error);
